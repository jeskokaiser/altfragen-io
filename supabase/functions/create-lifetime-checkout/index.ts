...
import { getPublishableKey } from '../_shared/supabaseKeys.ts';
...
const log = (step: string, details?: unknown) => {
  console.log(
    `[CREATE-LIFETIME-CHECKOUT] ${step}${details ? ` - ${JSON.stringify(details)}` : ''}`,
  );
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey || !stripeKey || !lifetimePriceId) {
