...
async function fetchBlocklist(): Promise<Set<string>> {
  try {
    ...
    logStep('Error fetching blocklist', {
...
async function getBlocklist(supabaseClient: any): Promise<Set<string>> {
  ...
  try {
    ...
    const { error: upsertError } = await supabaseClient.from('disposable_email_blocklist').upsert(
      {
        id: BLOCKLIST_ID,
    ...
    try {
      ...
      logStep('Fallback cache also failed', {
...
async function validateEmailDomain(
  email: string,
  supabaseClient: any,
): Promise<{ valid: boolean; domain: string | null; reason?: string }> {
