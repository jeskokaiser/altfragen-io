...
export const ExamCohortComparisonSection: React.FC<ExamCohortComparisonSectionProps> = ({
  examId,
  examName,
  subscribed,
}) => {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-lg font-semibold">Benchmarking</CardTitle>
      </CardHeader>
      <CardContent>
        {isCohortLoading && <p className="text-sm text-muted-foreground">Lade Benchmarking...</p>}

        {!isCohortLoading && (cohortError || !cohortStats) && (
          <p className="text-sm text-muted-foreground">
            Vergleichsdaten zu deiner Uni für diese Prüfung sind derzeit nicht verfügbar.
          </p>
        )}

        {!isCohortLoading && cohortStats && cohortStats.sampleSize < 5 && (
          <p className="text-sm text-muted-foreground">
            Zu wenig Daten für Benchmarking an deiner Uni für diese Prüfung.
          </p>
        )}
        ...
        {!isCohortLoading && cohortStats && cohortStats.sampleSize >= 5 && (
          <div className="relative">
            {!subscribed && (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center px-4 text-center">
                <div className="rounded-lg bg-background/90 shadow-md border px-4 py-3 max-w-xl space-y-2">
                  <p className="text-sm font-semibold">Benchmarking ist ein Premium-Feature.</p>
                  <p className="text-xs text-muted-foreground">
                    Sieh genau, wie du im Vergleich zu anderen Studierenden deiner Hochschule in
                    dieser Prüfung abschneidest – mit Perzentilen, Aktivitätsvergleich und
                    Score-Verteilung.
                  </p>
            ...
            <div
              className={
                !subscribed ? 'pointer-events-none select-none filter blur-md opacity-70' : ''
              }
            >
              <div className="space-y-6">
                <div className="space-y-1">
                  <p className="text-sm text-muted-foreground">
                    Vergleich mit anderen Studierenden deiner Hochschule für diese Prüfung
                    {universityName ? ` (${universityName})` : ''}.
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Basis: {cohortStats.sampleSize} aktive Nutzer in deiner Vergleichsgruppe. Fahre
                    mit der Maus über die Balken im Histogramm, um mehr Informationen zu erhalten.
                  </p>
                  <div className="flex items-start space-x-2 text-amber-700 text-md">
                    <Info className="h-5 w-5 mt-0.5 min-w-[1rem]" />
                    <span>
                      Hinweis: Übergangsweise sind hier noch Daten aus der Beta-Version vorhanden,
                      weshalb es zu Unstimmigkeiten in den Daten kommen kann. Sobald mehr Daten
                      vorhanden sind und die alten Daten entfernt wurden, wird dieser Hinweis
                      entfernt.
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <div className="text-lg text-muted-foreground">Score</div>
                    <div className="text-2xl font-bold">{cohortStats.userScore.toFixed(0)}</div>
                  </div>
                  ...
                  <div className="space-y-1 text-sm text-muted-foreground">
                    <div>Du bist im {cohortStats.userPercentile.toFixed(0)}. Perzentil.</div>
                    <div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <div className="text-lg text-muted-foreground">Beantwortete Fragen</div>
                    <div className="text-2xl font-bold">{cohortStats.userAnswered}</div>
                  </div>

                  {/* Answered Questions Graph */}
                  {cohortStats.answeredDistribution &&
                    cohortStats.answeredDistribution.length > 0 && (
                      <HistogramChart
                  ...
                  <div className="space-y-1 text-sm text-muted-foreground">
                    {cohortStats.userAnsweredPercentile != null && (
                      <div>
                        Du bist im {cohortStats.userAnsweredPercentile.toFixed(0)}. Perzentil.
                      </div>
                    )}
                    <div>
                      Mittelwert der Vergleichsgruppe: {cohortStats.cohortAnsweredMean.toFixed(0)}
                    </div>
                    {cohortStats.cohortAnsweredMedian != null && (
                      <div>
                        Median der Vergleichsgruppe: {cohortStats.cohortAnsweredMedian.toFixed(0)}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <div className="text-lg text-muted-foreground">Trefferquote</div>
                    <div className="text-2xl font-bold">
                      {cohortStats.userAnswered > 0
                        ? ((cohortStats.userCorrect / cohortStats.userAnswered) * 100).toFixed(0)
                        : 0}
                      %
                    </div>
                  </div>
                  ...
                  {typeof cohortStats.cohortAccuracyMean === 'number' &&
                    ...
                    cohortStats.accuracyDistribution.length > 0 && (
                      <HistogramChart
                        ...
                        userValue={
                          cohortStats.userAnswered > 0
                            ? (cohortStats.userCorrect / cohortStats.userAnswered) * 100
                            : 0
                        }
                        meanValue={(cohortStats.cohortAccuracyMean ?? 0) * 100}
                        medianValue={
                          cohortStats.cohortAccuracyMedian != null
                            ? cohortStats.cohortAccuracyMedian * 100
                            : null
                        }
                        sampleSize={cohortStats.sampleSize}
                        label="Trefferquote"
                    ...
                    {typeof cohortStats.cohortAccuracyMean === 'number' ? (
                      <>
                        {cohortStats.userAccuracyPercentile != null && (
                          <div>
                            Du bist im {cohortStats.userAccuracyPercentile.toFixed(0)}. Perzentil.
                          </div>
                        )}
                        <div>
                          Durchschnitt der Vergleichsgruppe (Mittelwert):{' '}
                      ...
                      <div>
                        Aggregierte Trefferquote der Vergleichsgruppe ist noch nicht verfügbar.
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <CollapsibleContent className="mt-3 space-y-4 text-sm text-muted-foreground">
                    <div className="rounded-lg border bg-muted/40 p-4 space-y-4">
                      <div>
                        ...
                        <p className="mb-2">
                          Jede Metrik (Score, Beantwortete Fragen, Trefferquote) wird in drei Teilen
                          dargestellt:
                        </p>
                        <ul className="list-disc list-inside space-y-1 ml-2">
                          ...
                          <li>
                            <strong>Histogramm:</strong> Die Verteilung aller Nutzer in deiner
                            Vergleichsgruppe. Fahre mit der Maus über die Balken, um Details zu
                            sehen. Die Markierungen zeigen:
                          </li>
                          <li>
                            <strong>Details:</strong> Dein Perzentil, Vergleichswerte (Mittelwert,
                            Median) und weitere Informationen
                          </li>
                        </ul>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="mb-2">
                          ...
                          <strong>Aktivität</strong> (15% Gewichtung) deiner Antworten. Dieses
                          Verfahren ist stabil, schwer zu manipulieren und produziert vergleichbare
                          Werte.
                        </p>
                        ...
                        <div className="space-y-2">
                          <div>
                            ...
                            <p className="mt-1">
                              Deine Trefferquote wird mit einem statistischen Verfahren (Bayesian
                              Schrumpfung) angepasst, um Zufallsschwankungen bei wenigen Antworten
                              auszugleichen.
                            </p>
                            ...
                            <p className="mt-1 text-xs">
                              Dabei ist: <strong>r</strong> = Anzahl richtige Antworten,{' '}
                              <strong>n</strong> = Anzahl beantwortete Fragen,
                              <strong>p₀</strong> = Durchschnittliche Trefferquote der
                              Vergleichsgruppe,
                              <strong>m</strong> = Prior-Stärke (75 Pseudo-Fragen)
                            </p>
                            <p className="mt-1 text-xs">
                              Wenn du nur wenige Fragen beantwortet hast, wird deine Quote zum
                              Durchschnitt der Vergleichsgruppe hin korrigiert. Mit mehr Antworten
                              nähert sich der Wert deiner tatsächlichen Trefferquote r/n an.
                            </p>
                          </div>

                          <div>
                            <strong className="text-foreground">
                              2. Aktivität (logarithmische Skalierung):
                            </strong>
                            <p className="mt-1">
                              Die Anzahl deiner beantworteten Fragen wird auf einer logarithmischen
                              Skala normalisiert, sodass frühe Übung zählt, aber Power-User nicht
                              überproportional dargestellt werden.
                            </p>
                            <p className="mt-1 text-xs font-mono bg-muted/60 p-2 rounded">
                            ...
                            <p className="mt-1 text-xs">
                              ...
                              <strong>n_ref</strong> = Referenzwert (95. Perzentil der aktiven
                              Nutzer)
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-xs">
                              Der Score liegt zwischen 0 und 100 Punkten. Qualität hat 85%
                              Gewichtung, Aktivität 15% Gewichtung.
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <h4 className="font-semibold text-foreground mb-2">Perzentil</h4>
                        <p>
                          Das Perzentil zeigt deine Position innerhalb der Vergleichsgruppe. Ein
                          Perzentil von 75 bedeutet, dass du besser abschneidest als 75% der anderen
                          Studierenden in deiner Hochschule für diese Prüfung. Das Perzentil wird
                          für alle drei Metriken (Score, Beantwortete Fragen, Trefferquote) separat
                          berechnet.
                        </p>
                        <p className="mt-1 text-xs">
                          Berechnung: (Anzahl der Nutzer mit niedrigerem Wert) / (Gesamtanzahl - 1)
                          × 100
                        </p>
                      </div>

                      <div>
                        <h4 className="font-semibold text-foreground mb-2">Histogramme</h4>
                        <p className="mb-2">
                          Die Histogramme zeigen die tatsächliche Verteilung aller Nutzer in deiner
                          Vergleichsgruppe:
                        </p>
                        <ul className="list-disc list-inside space-y-1 ml-2">
                          <li>
                            <strong>Score-Histogramm:</strong> Verteilung der Scores (0-100 Punkte)
                            in 5-Punkte-Buckets. Jeder Balken zeigt, wie viele Nutzer in diesem
                            Score-Bereich liegen.
                          </li>
                          <li>
                            <strong>Beantwortete Fragen:</strong> Verteilung der Anzahl
                            beantworteter Fragen mit dynamischen Bucket-Größen (5, 10, 25, 50 oder
                            100 Fragen je nach Datenbereich).
                          </li>
                          <li>
                            <strong>Trefferquote:</strong> Verteilung der Trefferquoten (0-100%) in
                            5%-Buckets.
                          </li>
                        </ul>
                        <p className="mt-2">
                          Fahre mit der Maus über die Balken, um Details zu jedem Bereich zu sehen:
                          Anzahl der Nutzer, prozentualer Anteil der Gruppe und Vergleich zum
                          Mittelwert.
                        </p>
                      </div>

                      <div>
                        <h4 className="font-semibold text-foreground mb-2">Vergleichswerte</h4>
                        <ul className="list-disc list-inside space-y-1 ml-2">
                          <li>
                            <strong>Mittelwert:</strong> Der Durchschnittswert aller Nutzer in der
                            Vergleichsgruppe (arithmetisches Mittel)
                          </li>
                          <li>
                            <strong>Median:</strong> Der Wert, der die Gruppe in zwei Hälften teilt
                            – 50% der Nutzer haben einen höheren, 50% einen niedrigeren Wert. Der
                            Median ist robuster gegen Ausreißer als der Mittelwert.
                          </li>
                          <li>
                            <strong>Min/Max:</strong> Die niedrigsten und höchsten Werte in der
                            Vergleichsgruppe
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="pt-2 border-t">
                        <p className="text-xs">
                          <strong>Hinweis:</strong> Die Vergleichsgruppe umfasst alle Nutzer deiner
                          Hochschule, die Fragen mit dem gleichen Prüfungsnamen (
...
const BellCurveChart: React.FC<BellCurveChartProps> = ({ cohort }) => {
  ...
  const userX = scaleX(Math.min(Math.max(cohort.userScore, displayMinX), displayMaxX || 100));
  ...
  const getBucketInfo = (barIndex: number) => {
    ...
    const percentage = cohort.sampleSize > 0 ? (bucket.count / cohort.sampleSize) * 100 : 0;
      ...
      <div className="w-full overflow-hidden rounded-md border bg-muted/40">
        <div className="relative w-full" style={{ aspectRatio: `${width} / ${height}` }}>
          ...
          >
            ...
            <circle cx={meanX} cy={paddingY + 8} r={3.5} fill="hsl(142, 76%, 36%)" />
            ...
            <circle cx={userX} cy={paddingY + 8} r={4} fill="hsl(var(--primary))" />

            {/* X-axis labels */}
            <text x={paddingX} y={height - 4} fontSize={10} fill="currentColor">
              Niedriger Score
            </text>
            <text
              x={width - paddingX}
                  ...
                  <TooltipContent side="top" className="max-w-xs" sideOffset={8}>
                    <div className="space-y-1.5">
                      <div className="font-semibold">
                        Score-Bereich: {bucket.bucketMin.toFixed(0)} - {bucket.bucketMax.toFixed(0)}{' '}
                        Punkte
                      </div>
                      <div className="text-xs space-y-0.5">
                        <div>
                          <span className="font-medium">{bucket.count}</span> Nutzer in diesem
                          Bereich
                        </div>
                        <div>
                          {bucketInfo ? (
                            <>
                              <span className="font-medium">
                                {bucketInfo.percentage.toFixed(1)}%
                              </span>{' '}
                          ...
                          <div className="pt-1 border-t">
                            {bucketInfo.diffFromMean > 0 ? (
                              ...
                              <span className="text-red-600">
                                {Math.abs(bucketInfo.diffFromMean).toFixed(0)} Punkte unter dem
                                Mittelwert
                              </span>
                            )}
                          </div>
      ...
      <div className="w-full overflow-hidden rounded-md border bg-muted/40">
        <div className="relative w-full" style={{ aspectRatio: `${width} / ${height}` }}>
          ...
          >
            ...
            <circle cx={meanX} cy={paddingY + 6} r={3} fill="hsl(142, 76%, 36%)" />
            ...
            <circle cx={userX} cy={paddingY + 6} r={3.5} fill="hsl(var(--primary))" />
            ...
            {/* X-axis labels */}
            <text x={paddingX} y={height - 4} fontSize={9} fill="currentColor">
              Niedrig
            </text>
            <text
              x={width - paddingX}
            ...
            return (
              <Tooltip key={`tooltip-${idx}`}>
                ...
                <TooltipContent side="top" className="max-w-xs" sideOffset={8}>
                  <div className="space-y-1.5">
                    <div className="font-semibold">
                      {label}: {bucket.bucketMin.toFixed(0)} - {bucket.bucketMax.toFixed(0)}
                      {unit}
                    </div>
                    <div className="text-xs space-y-0.5">
                      <div>
                      ...
                      <div>
                        {bucketInfo ? (
                          <>
                            <span className="font-medium">{bucketInfo.percentage.toFixed(1)}%</span>{' '}
                            der Vergleichsgruppe
                          </>
                        ) : null}
                      </div>
                      {bucketInfo && Math.abs(bucketInfo.diffFromMean) > 0.1 && (
                        <div className="pt-1 border-t">
                          {bucketInfo.diffFromMean > 0 ? (
                            <span className="text-emerald-600">
                              {bucketInfo.diffFromMean.toFixed(0)}
                              {unit} über dem Mittelwert
                            </span>
                          ) : (
                            <span className="text-red-600">
                              {Math.abs(bucketInfo.diffFromMean).toFixed(0)}
                              {unit} unter dem Mittelwert
                            </span>
                          )}
                        </div>
                      )}
