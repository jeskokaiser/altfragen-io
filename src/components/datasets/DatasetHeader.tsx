...
const DatasetHeader: React.FC<DatasetHeaderProps> = ({
  ...
  return <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-start">
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
        {hasUnclearQuestions && <Button variant="outline" onClick={handleUnclearClick} className="w-full sm:w-auto" size={isMobile ? "sm" : "default"}>
            <AlertCircle className="mr-2 h-4 w-4" />
            Unklare Fragen ({unclearQuestions.length})
          </Button>}
        <Button onClick={() => onStartTraining(questions)} className="w-full sm:w-auto" size={isMobile ? "sm" : "default"}>
