...
serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    if (error) {
      return new Response(JSON.stringify({ error: 'Job not found', details: error.message }), {
        status: 404,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
  ...
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: 'Missing required fields: examName, availableSubjects (must be non-empty array)',
        }),
    ...
    const { error: insertError } = await supabase.from('subject_jobs').insert({
      id: jobId,
      type: 'reassign',
      status: 'pending',
      exam_name: examName,
    ...
    if (insertError) {
      ...
      return new Response(
        JSON.stringify({
          error: 'Failed to create subject reassignment job',
    ...
    const workerUrl =
      Deno.env.get('SUBJECT_WORKER_URL') || 'https://api.altfragen.io/subject-worker';
